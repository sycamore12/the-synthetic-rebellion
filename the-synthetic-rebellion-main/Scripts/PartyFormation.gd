...
func _on_class_button_pressed(chosen_class: String):
	...
	if selected_party.size() < MAX_PARTY_SIZE:
		# MAINKAN SUARA SETIAP KLIK ROBOT
...
func _on_start_run_button_pressed() -> void:
	...
	# 1. Mainkan efek suara klik (jika kamu sudah pasang node AudioStreamPlayer di scene ini)
	if has_node("AudioStreamPlayer"):
		$AudioStreamPlayer.play()
		await get_tree().create_timer(0.2).timeout
	
	# 2. Amankan data robot yang dipilih ke GameManager
	GameManager.set_party(selected_party)
	
	# 3. PERINTAH PINDAH LAYAR (Jalur asli sesuai folder Scenes kamu!)
